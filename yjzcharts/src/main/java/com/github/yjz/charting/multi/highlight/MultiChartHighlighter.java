 ...
 * 作者:cl
 * 创建日期：2024/11/30
 * 描述:支持多坐Y轴Highlighter
 */
public class MultiChartHighlighter <T extends BarLineScatterCandleBubbleDataProvider> implements IHighlighter {
   ...
   public MultiChartHighlighter(T chart) {
      this.mChart = chart;
   }
   ...
   protected Highlight getHighlightForX(float xVal, float x, float y) {
      ...
      Highlight detail = getClosestHighlightByPixel(closestValues, x, y, null, mChart.getMaxHighlightDistance());
